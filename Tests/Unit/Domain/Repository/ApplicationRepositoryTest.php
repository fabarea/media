...
class ApplicationRepositoryTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase {
	...
	public function findByReturnsSameNumberOfRecordsAsNumberOfFakeRecords() {
		...
		$this->assertEquals($this->numberOfFakeApplications, count($this->fixture->findBy($match)));
	}
	...
	public function countByReturnsSameNumberOfRecordsAsNumberOfFakeRecords() {
		$match = new \TYPO3\CMS\Media\QueryElement\Match();
		$match->addMatch('tx_phpunit_is_dummy_record', '1');
		$this->assertEquals($this->numberOfFakeApplications, $this->fixture->countBy($match));
	}
