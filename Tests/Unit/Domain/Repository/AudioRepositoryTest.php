...
/**
 * Test case for class \TYPO3\CMS\Media\Domain\Repository\AudioRepository.
 *
 * @author Fabien Udriot <fabien.udriot@typo3.org>
 * @package TYPO3
 * @subpackage media
 */
class AudioRepositoryTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase {
	 ...
	 * @var int
	 */
	private $numberOfFakeAudios = 4;

	/**
	 * @var \TYPO3\CMS\Media\Domain\Repository\AudioRepository
	 */
	private $fixture;

	public function setUp() {
		$this->testingFramework = new Tx_Phpunit_Framework('sys_file');
		$this->fixture = new \TYPO3\CMS\Media\Domain\Repository\AudioRepository();
		...
		$this->populateFileTableWithAudios();
	}
	 ...
	 * @test
	 */
	public function findAllAudioReturnsTheGreaterOrEqualThanNumberOfFakeAudios() {
		$this->assertGreaterThanOrEqual($this->numberOfFakeAudios, count($this->fixture->findAll()));
	}
	...
	public function findFilteredReturnsSameNumberOfRecordsAsNumberOfFakeRecords() {
		...
		$this->assertEquals($this->numberOfFakeAudios, count($this->fixture->findFiltered($filter)));
	}
	...
	public function countFilteredReturnsSameNumberOfRecordsAsNumberOfFakeRecords() {
		$filter = new \TYPO3\CMS\Media\QueryElement\Filter();
		$filter->addConstraint('tx_phpunit_is_dummy_record', '1');
		$this->assertEquals($this->numberOfFakeAudios, $this->fixture->countFiltered($filter));
	}

	/**
	 * @test
	 */
	public function findByUidCanReturnTheLastInsertedAudio() {
		$actual = $this->fixture->findByUid($this->lastInsertedUid);
		$this->assertTrue($actual instanceof \TYPO3\CMS\Media\Domain\Model\Audio);
	}

	/**
	 * @test
	 */
	public function findByTitleReturnsTheSameNumberAsFakeAudioRecords() {
		$this->assertEquals($this->numberOfFakeAudios, count($this->fixture->findByTitle($this->fakeTitle)));
	}
	...
	public function countByTypeReturnsTheSameNumberAsFakeRecords() {
		$this->assertEquals($this->numberOfFakeAudios, $this->fixture->countByTitle($this->fakeTitle));
	}
	 ...
	 * Populate DB with default records
	 */
	private function populateFileTableWithAudios() {

		for ($index = 0; $index < $this->numberOfFakeAudios; $index++) {
			$this->lastInsertedIdentifier = uniqid();
			$this->lastInsertedUid = $this->testingFramework->createRecord(
				'sys_file',
				array(
