 ...
 */
class VariantRenderer extends GridRendererAbstract {
	...
	public function render() {
		...
		if (!empty($variants)) {
			...
			foreach ($variants as $variant) {
				...
				// Count usage
				$softImageReferencesCount = $this->countSoftImageReferences($variant);

				$_result .= sprintf($this->getVariantTemplate(),
					$variant->getUid(),
					$softImageReferencesCount,
					$softImageReferencesCount > 1 ? 's' : '',
					Path::getRelativePath($variant->getPublicUrl()),
					...
					$icon,
					$variant->getProperty('width'),
					$variant->getProperty('height')
				);
			}
