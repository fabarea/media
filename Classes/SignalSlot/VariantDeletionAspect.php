...
class VariantDeletionAspect {
	...
	protected function getVariantRepository() {
		return GeneralUtility::makeInstance('TYPO3\CMS\Media\Domain\Repository\VariantRepository');
	}
	 ...
	 * @return void
	 */
	public function removeFromRepository(FileInterface $file) {

		// remove file from repository
		if ($file instanceof File) {

			$asset = ObjectFactory::getInstance()->createObject($file->getProperties());
			if (!$asset->getIsVariant()) {
				foreach ($asset->getVariants() as $variant) {
					$this->getDatabaseConnection()->exec_DELETEquery('sys_file_variants', 'variant_resource = ' . $variant->getUid());
					$variant->delete();
				}
			}
		}
